$(function () {
    ...
    function validatePhone() {
        var regex = /(^(\+375)\d{9})|(^(80)\d{9})/;
        var val = $('.tel', this).val();
        ...
        if (validatePhone.apply($(this)) != -1) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $form.after('<div class="form-response form-response_success">Спасибо! В ближайшее время с вами свяжется менеджер.</div>');
                    $('.form-response').delay(3000).fadeOut(800, function () {
                        $(this).remove();
                    });
                },
            ...
            $form.after('<div class="form-response form-response_error">Введите телефон в формате +375XXXXXXXXX или 80XXXXXXXXX</div>');
            $('.form-response').delay(3000).slideToggle(300, function () {
                $(this).remove();
            });
        }
    }); // обратный звонок
    ...
    function clearAll() {
        $('#configurator-table td').not('.component').html('');
    ...
    $('#add-cmpt').on('click', function () {
        ...
        $('#configurator-table tr').each(function () {
    ...
    $('#configurator-table tbody').on('click', '.del-cmpt', function () {
    ...
    $('#configurator-table tbody').on('click', '.add-cmpt', function () {

        // наценка для корпотела
        var extra = 0.1;
        ...
        $.getJSON(jsonSource, { cache: false }, function (data) {
                ...
                if (curComponent == 'platform') {
                    ...
                    for (i = 0; i < curComponentArr.length; i++) {
                        ...
                        if (curRelevant.indexOf(curPlatform) + 1) {
                            ...
                            $(subCat).after('<tr class="component-item" data-relevant="' + curRelevant + '"><td class="cmpt-name">' + curComponentArr[i].name + curLink + '</td><td class="cmpt-shortname" style="display:none">' + curComponentArr[i].shortname + '</td><td class="cmpt-price">' + (curComponentArr[i].price + curComponentArr[i].price * extra) + '</td><td class="cmpt-avail">' + curComponentArr[i].availability + '</td><td class="choose"><button class="btn btn-primary choose-cmpt" data-dismiss="modal">Выбрать</button></td></tr>');
                        }
                    }
            ...
            // Показывать все компоненты
                ...
                for (i = 0; i < curComponentArr.length; i++) {
                    ...
                    // выведем элементы после строки с подкатегорией
                    $(subCat).after('<tr data-relevant="' + curRelevant + '"><td class="cmpt-name">' + curComponentArr[i].name + curLink + '</td><td class="cmpt-shortname" style="display:none">' + curComponentArr[i].shortname + '</td><td class="cmpt-price">' + (curComponentArr[i].price + curComponentArr[i].price * extra) + '</td><td class="cmpt-avail">' + curComponentArr[i].availability + '</td><td class="choose"><button class="btn btn-primary choose-cmpt" data-dismiss="modal">Выбрать</button></td></tr>');
                }
            }
    ...
    $('#send-config').submit(function (e) {
        ...
        if (validatePhone.apply($(this)) != -1) {
            ...
            getOrderNumber(function (orderNum) {
                ...
                var $data = $form.serialize() + '&config=' + $config + '&price=' + $price + '&number=' + $number + '&num=' + configNum + '&avail=' + $avail;
                $.ajax({
                    type: $form.attr('method'),
                    url: $form.attr('action'),
                    data: $data,
                    success: function (response) {
                        ...
                        $form.append('<div class="form-response form-response_success">Спасибо! В ближайшее время с вами свяжется менеджер.</div>');
                        $('.form-response').delay(3000).fadeOut(800, function () {
                            $(this).remove();
                        });
            ...
            $form.append('<div class="form-response form-response_error">Введите телефон в формате +375XXXXXXXXX или 80XXXXXXXXX</div>');
            $('.form-response').delay(3000).slideToggle(300, function () {
                $(this).remove();
            });
        }
